...
func TestCommitAndPushWithRetry_RefreshFailureStopsRetrying(t *testing.T) {
	...
	origBaseDelay := pushRetryBaseDelay
	pushRetryBaseDelay = 20 * time.Millisecond
	t.Cleanup(func() { pushRetryBaseDelay = origBaseDelay })

	result, err := policy.ApplyPolicies(ctx, sm.WorkDirectory(), obj, policies)
	g.Expect(err).ToNot(HaveOccurred())

	// Break connectivity so both the push and the subsequent refresh fail -
	// there's no conflict to recover from, just a dead remote. Retrying is
	// unconditional now, but the loop must still give up after the first
	// failed refresh instead of burning the whole pushRetryMaxAttempts
	// budget on a remote that will never come back.
	gitServer.StopHTTP()
	...
	g.Expect(err).To(HaveOccurred())
	// One backoff sleep, then RefreshToRemote fails immediately - not the
	// full pushRetryMaxAttempts budget.
	g.Expect(elapsed).To(BeNumerically(">=", pushRetryBaseDelay))
	g.Expect(elapsed).To(BeNumerically("<", 3*pushRetryBaseDelay))
}
...
func TestCommitAndPushWithRetry_FeatureGateDisabled(t *testing.T) {
	...
	r := &ImageUpdateAutomationReconciler{features: map[string]bool{features.GitPushRetryOnFailure: false}}
	_, err = r.commitAndPushWithRetry(ctx, sm, obj, policies, result, nil)
	g.Expect(err).To(HaveOccurred())
	// Feature gate off: a single CommitAndPush call, no refresh-and-retry.
